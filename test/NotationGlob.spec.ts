...
describe('NotationGlob', () => {
  const { isValid, hasMagic, toRegExp, split, compare, sort, normalize, union, create } =
    NotationGlob;

  // private methods
  const _inspect = (NotationGlob as UnknownObject)._inspect;
  const _covers = (NotationGlob as UnknownObject)._covers;
  const _intersect = (NotationGlob as UnknownObject)._intersect;
  ...
  test('.hasMagic()', () => {
    ...
    // @ts-expect-error: testing boolea
  ...
  test('.toRegExp()', () => {
    ...
    expect(toRegExp('!x.*["x.*"][1]').source).toEqual(
      '^x\\.' + reVAR + '\\["x\\.\\*"\\]\\[1\\]' + reREST
    );
  });
  ...
  test('.split()', () => {
    ...
    expect(split('*.x.y[*][1].z["*.x"].a')).toEqual([
      '*',
      'x',
      'y',
      '[*]',
  ...
  test('#covers(), ._covers()', () => {
    const cov = (globA: string, globB: string | string[] | NotationGlob): boolean =>
      create(globA).covers(globB);
  ...
  test('#intersect(), ._intersect()', () => {
    const intersect = (globA: string, globB: string, restrictive: boolean = false): string | null =>
      create(globA).intersect(globB, restrictive);
  ...
  test('.normalize() restrictive = false', () => {
    const norm = (globs: string | string[]): string[] => normalize(globs, false);
    ...
    expect(norm(['[*]', '![*][1]', '![0][1]', '![0][2]', '![1][2][0]'])).toEqual([
      '[*]',
      '![0][2]',
      '![*][1]',
      '![1][2][0]'
    ...
    expect(norm(['*', 'user', 'user.id', '!*.id', 'video.id'])).toEqual([
      '*',
      '!*.id',
      'user.id',
      'video.id'
    ]);
    expect(norm(['video.id', '!*.id', 'user', 'user.id', '*'])).toEqual([
      '*',
      '!*.id',
      'user.id',
      'video.id'
    ]); // re-ordered version of above
    expect(norm(['*', 'user', '!*.id', 'video.id'])).toEqual(['*', '!*.id', 'video.id']);

    expect(norm(['*', 'user.id', '!*.id', 'video.id'])).toEqual([
      '*',
      '!*.id',
      'user.id',
      'video.id'
    ...
    expect(
      norm(['*', '!id', 'name', 'car.model', '!car.*', 'id', 'name', 'user', '!user.pwd'])
    ).toEqual(['*', '!id', '!car.*', 'car.model', '!user.pwd']);

    expect(norm(['*', '!id', 'car.model', '!car.*', '!user.pwd'])).toEqual([
      '*',
      '!id',
      '!car.*',
      'car.model',
    ...
    expect(norm(['!x', 'c[1]', '!c[*]', '*', '!d.e'])).toEqual([
      '*',
      '!x',
      '!c[*]',
      'c[1]',
    ...
    expect(norm(['!id', 'name', 'car.model', '!car.*', 'id', '!email'])).toEqual([
      'name',
      'car.model'
    ]);

    expect(norm(['!y.*', 'x.x[1][0][*]', '*.x[*]', '!x.x[2][*]', 'a.b', 'c[*][1]'])).toEqual([
      '*.x',
      'a.b',
      'c[*][1]',
      '!x.x[2][*]'
  ...
  test('.normalize() restrictive = true', () => {
    const norm = (globs: string | string[]): string[] => normalize(globs, true);
    ...
    expect(norm(['!x', 'c[1]', '!c[*]', '*', '!d.e'])).toEqual(['*', '!x', '!c[*]', '!d.e']);

    expect(norm(['!id', 'name', 'car.model', '!car.*', 'id', '!email'])).toEqual(['name']);
    ...
    expect(norm(['!x', '*.b'])).toEqual(['*.b', '!x.b']);
    expect(norm(['!x.b.c', '*.b'])).toEqual(['*.b', '!x.b.c']);
  ...
  test('.normalize() » issue #7', () => {
    const globs = ['!pass', '!password.prop', '*', '!password', '!password_reset_code'];
    // console.log(normalize(globs));
    expect(normalize(globs)).toEqual(['*', '!pass', '!password', '!password_reset_code']);
  });
  ...
  test('.union() restrictive = false', () => {
    ...
    expect(uni(['*', 'a', 'b', '!id', '!x.*'], ['*', '!b', 'id', '!pwd', 'x.o'])).toEqual(['*']);
    ...
    expect(uni(['*', 'a', 'b[2]', '!id', '!x[*]'], ['*', '!b[*]', 'id', '!x[4]', 'x[1]'])).toEqual([
      '*',
      '!x[4]'
    ]);

    expect(uni(['*[*]', '!a[1]', '!x[*]'], ['*', 'a[1]', '!b[*]', '!x[*]', 'x[0]'])).toEqual([
      '*',
      '!x[*]',
      'x[0]'
    ]);

    expect(uni(['*', '!a[*]', '!x[*]'], ['*', 'a[*]', '!b', '!x[*].*', 'x[1]'])).toEqual([
      '*',
      '!x[*].*'
    ]);
  ...
  test('.union() restrictive = true', () => {
    ...
    expect(uni(['*', '!id', '!x.*'], ['*', 'id', '!pwd', '!x.*', 'x.o'])).toEqual(['*', '!x.*']);
  ...
  test('.union() check mutation, order', () => {
    ...
    expect(union(globA, globB, true)).toEqual([
      'bar',
      'foo.qux',
      '!bar.id',
      'foo.bar.baz',
    ...
    expect(union(['*', 'a[*]', '!b[2]', '!x[*]', 'o'], ['*', 'b[2]', '!pwd', 'x[5]', 'o'])).toEqual(
      ['*']
    );
