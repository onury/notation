...
describe('utils', () => {
  ...
  test('.hasOwn(), .cloneDeep()', () => {
    ...
    Obj.prototype.hasOwnProperty = (): boolean => true;
    ...
    const copy = utils.cloneDeep(o);
    expect(copy).toEqual(o);
    expect(copy === o).toEqual(false);
    const copy2 = utils.cloneDeep([o]);
    expect(copy2).toEqual([o]);
  });
  ...
  test('.each(), .eachRight()', () => {
    ...
    out = [];
    utils.each(a, (value, index): void | false => {
      if (index <= 1) {
        out.push(value);
      } else {
        return false;
    ...
    out = [];
    utils.eachRight(a, (value, index): void | false => {
      if (index > 1) {
        out.push(value);
      } else {
        return false;
  ...
  test('.eachItem()', () => {
    ...
    let out: unknown[] = [];
    utils.eachItem(c1, (item: unknown, index: number, collection: unknown[]) => {
      out.push([index, item]);
    ...
    out = [];
    utils.eachItem(c2, (item: unknown, key: string, collection: UnknownObject) => {
      out.push([key, item]);
