...
describe('Notation', () => {
  const { create, first, last, parent, countNotes, isValid, split, join, eachNote } = Notation;
  ...
  test('NotationError', () => {
    ...
    try {
      throw new NotationError(errorMessage);
    } catch (error) {
      // console.log(error);
      expect((error as Error).name).toEqual('NotationError');
    ...
    expect(new NotationError() instanceof NotationError).toEqual(true);
  });
  ...
  test('.countNotes()', () => {
    ...
    expect(countNotes('a.b.c')).toEqual(3); // alias
  ...
  test('constructor(), .create()', () => {
    ...
    expect(new Notation({}).set('x', '123').value.x).toEqual('123');
    // { length: 0 }
  ...
  test('#flatten(), #exapand()', () => {
    const nota = new Notation(structuredClone(o));
    ...
    expect(expanded.account.likes.length).toEqual(4);
  });
  ...
  test('#each(), #eachValue()', () => {
    const assets = {
      ...
      car: {
        ...
        alternate: {
          cars: [{ brand: 'Dodge', model: 'Charger', year: 1970 }]
        }
      }
    };
    ...
    nota.each((notation) => {
      result.push(notation);
      return result.length < 4 ? undefined : false; // should break out on 4
    });
    expect(result.length).toEqual(4);

    let c = 0;
    nota.eachValue('car.alternate.cars[0]', (levelValue, _levelNotation, _note, index) => {
      c++;
      if (index === 0) expect((levelValue as UnknownObject).model).toEqual('Mustang');
      if (index === 1) return false;
      return;
    });
    expect(c).toEqual(2);

    // Test function instance / prototype

    function A() {} // eslint-disable-line
    A.prototype.x = 1; // x is NOT own enumerable
    ...
    // Test class instance property

    class B {
      x: number = 1;
    } // x is own enumerable
    nota = new Notation(new B());
    c = 0;
    nota.eachValue('x', (levelValue, levelNotation, note, index) => {
      c++;
      if (index === 0) expect(levelValue).toBeDefined();
    });
    expect(c).toEqual(1);
  });
  ...
  test('#get()', () => {
    let nota = new Notation(structuredClone(o));
  ...
  test('#remove()', () => {
    ...
    create(assets).remove('car.model'); // alias
  ...
  test('#clone()', () => {
    ...
    const obj: UnknownObject = { a: { b: { c: [1, 2, {}] } } };
  ...
  test('#merge(), #separate()', () => {
    ...
    expect(Array.isArray(merged.newkey.p3.val)).toEqual(true);

    // @ts-expect-error: testing number
    expect(() => create().merge(1)).toThrow();
    // below will fail bec. {} can be merged with array
    expect(() => create().merge([])).toThrow(); // TODO: add more tests for merge()
    expect(() => create([]).merge({ x: 1 })).toThrow(); // TODO: add more tests for merge()
    // @ts-expect-error: testing null
    expect(() => create().merge(null)).toThrow();

    const separated = nota.separate(['newkey.p1', 'newkey.p2', 'newkey.p3.val']);
    expect(separated.key).toBeUndefined();
    expect(separated.newkey.p1).toEqual(13);
    expect(separated.newkey.p2).toEqual(false);
    expect(Array.isArray(separated.newkey.p3.val)).toEqual(true);
  ...
  test('#rename()', () => {
    ...
    expect(typeof obj.company.loc.geo.lat === 'number').toEqual(true);
