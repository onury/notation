...
var utils = {
    ...
    splitNotation: function splitNotation(notation) {
        ...
        for (var i = 0; i < chars.length; i++) {
            ...
            if (typeof note === 'string' && note.length) notes.push(note);
        }
        return notes;
    },
...
var Notation = function () {
    ...
    _createClass(Notation, [{
                ...
                if (_utils2.default.isObject(prop)) {
                    ...
                    try {
                        var _loop = function _loop() {
                            ...
                            if (!(_utils2.default.isObject(p) || _utils2.default.isArray(p))) {
                                callback.call(_this, subKey, subKey, p, o);
                            } else {
                                N = new Notation(p);
                                N.each(function (notation, nKey, value, prop) {
                        ...
                        for (var _iterator = prop.entries()[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                            _loop();
                        }
                    } catch (err) {
                        _didIteratorError = true;
            ...
            _utils2.default.each(globs, function (globNotation, index, array) {
                ...
                if (normalized.indexOf('*') < 0) {
                    if (g.isNegated) {
                        ...
                        if (endStar) filtered.set(normalized, {}, true);else if (endArrStar) filtered.set(normalized, [], true);
                    } else {
        ...
        value: function copyTo(destination, notation) {
            ...
            if (!(_utils2.default.isObject(destination) || _utils2.default.isArray(destination))) throw new _notation4.default(ERR.DEST);
