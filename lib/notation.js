...
var utils = {
    ...
    isArrIndex: function isArrIndex(note) {
        return (/^\[\d+\]$/.test(note)
        );
    },
    splitNotation: function splitNotation(notation) {
        var notes = [];
        var chars = notation.split('');
        var start = 0;
        var bracketCount = 0;
    ...
    concatNotes: function concatNotes(notes) {
        var _this = this;

        return notes.reduce(function (acc, note) {
            return acc + (_this.isArrIndex(note) ? note : '.' + note);
        });
    },
    getIndexNumber: function getIndexNumber(notation) {
        return +notation.replace(/[\[\]]/g, '');
    },
    removeEmptyArraySpots: function removeEmptyArraySpots(obj) {
        var _this2 = this;
        ...
        if (this.isObject(obj)) {
            ...
            return obj.filter(function (e) {
                ...
                return _this2.removeEmptyArraySpots(e);
            });
        } else {
            return obj;
        }
...
var Notation = function () {
    ...
    _createClass(Notation, [{
        ...
        value: function each(callback) {
            ...
            var isArray = _utils2.default.isArray(o);

            _utils2.default.each(keys, function (key, index, list) {
                // this is preserved in arrow functions
                var prop = o[key];
                var keyName = isArray ? '[' + key + ']' : key;
                var N = void 0;
                if (_utils2.default.isObject(prop)) {
                    N = new Notation(prop);
                    N.each(function (notation, nKey, value, prop) {
                        var subKey = keyName + '.' + notation;
                        callback.call(N, subKey, nKey, value, o);
                    });
                } else if (_utils2.default.isArray(prop) && prop.length) {
                    handleArrayEach(_this, callback, prop, keyName);
                } else {
                    callback.call(_this, keyName, keyName, prop, o);
                }
            });
            ...
            function handleArrayEach(self, cb, arr, key) {
                ...
                var _loop = function _loop(i) {
                    var subkey = key + '[' + i + ']';
                    if (_utils2.default.isObject(arr[i])) {
                        var N = new Notation(arr[i]);
                        N.each(function (notation, nKey, value, prop) {
                            var _subKey = subkey + '.' + notation;
                            cb.call(N, _subKey, nKey, value, o);
                        });
                    } else if (_utils2.default.isArray(arr[i])) {
        ...
        value: function eachValue(notation, callback) {
            ...
            Notation.eachNote(notation, function (levelNotation, note, index, list) {
                var noteKey = _utils2.default.isArrIndex(note) ? _utils2.default.getIndexNumber(note) : note;
                level = _utils2.default.hasOwn(level, noteKey) ? level[noteKey] : undefined;
        ...
        value: function inspect(notation) {
            ...
            Notation.eachNote(notation, function (levelNotation, note, index, list, isArray) {
                note = _utils2.default.isArrIndex(note) ? _utils2.default.getIndexNumber(note) : note;
        ...
        value: function inspectRemove(notation) {
            ...
            var o = void 0,
                lastNote = void 0;
            var notes = _utils2.default.splitNotation(notation);
            if (notes.length === 1) {
                lastNote = notation;
                o = this._source;
            } else {
            ...
            var result = void 0;
            lastNote = _utils2.default.isArrIndex(lastNote) ? _utils2.default.getIndexNumber(lastNote) : lastNote;
        ...
        value: function set(notation, value) {
            ...
            var concatArrays = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
            ...
            var level = this._source,
                last = void 0;
            Notation.eachNote(notation, function (levelNotation, note, index, list, isArray) {
                note = _utils2.default.isArrIndex(note) ? +note.replace(/[\[\]]/g, '') : note;
        ...
        value: function merge(notationsObject) {
            ...
            var value = void 0;
            var N = new Notation(notationsObject).flatten();
            _utils2.default.each(Object.keys(N._source), function (notation, index, obj) {
                // console.log(notation, index, obj);
                // this is preserved in arrow functions
                value = N._source[notation];
                _this2.set(notation, value, overwrite, true);
            });
            return this;
        }
                    ...
                    Notation.eachNote(originalNotation, function (levelNotation, note, index, list) {
                        if (g.test(levelNotation)) {
                            ...
                            if (list.length - 1 > index) {
                                var nextLevelNotation = _utils2.default.concatNotes([levelNotation, list[index + 1]]);
        ...
        value: function copyTo(destination, notation) {
            ...
            if (!(_utils2.default.isObject(destination) || _utils2.default.isArray(destination))) {
                throw new _notation4.default(ERR.DEST);
            }
        ...
        value: function extract(notation, newNotation) {
            ...
            o = _utils2.default.removeEmptyArraySpots(o);
            return o;
        }
        /**
         *  Alias for `#extract`
        ...
        value: function countNotes(notation) {
            ...
            return _utils2.default.splitNotation(notation).length;
        }
        /**
        ...
        value: function first(notation) {
            ...
            return _utils2.default.splitNotation(notation)[0];
        }
        ...
        value: function last(notation) {
            ...
            // return notation.replace(/.*\.([^\.]*$)/, '$1');
            return _utils2.default.splitNotation(notation).pop();
        }
        ...
        value: function parent(notation) {
            ...
            var notes = _utils2.default.splitNotation(notation);
            return notes.length > 1 ? _utils2.default.concatNotes(notes.slice(0, -1)) : null;
        }
        ...
        value: function eachNote(notation, callback) {
            ...
            _utils2.default.each(notes, function (note, index, list) {
                ...
                levelNotation = _utils2.default.concatNotes(levelNotes);
                var isArray = !!(notes[index + 1] && _utils2.default.isArrIndex(notes[index + 1]));
